...
from dhravya_model import rnn_model
...
def train_step():  # Training step for each epoch
    ...
    while True:
        ...
        loss = loss_fn(
            torch.squeeze(output), torch.squeeze(target_sequence)
        )  # calculate the loss
        ...
        wandb.log(
            {"loss": training_loss / n_steps}
        )  # Log the loss to weights and biases dashboard
...
def generate_sample():  # generate sample sequence after each epoch
    ...
    while True:
        output, hidden_state = rnn_model(
            test_input_seq, hidden_state
